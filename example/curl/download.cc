...
template<int N>
bool startWith(const leaf::string& str, const char (&prefix)[N]) {
...
class Piece : boost::noncopyable {
 public:
  Piece(const curl::RequestPtr& req,
    ...
    LOG_DEBUG << "start with offet: " << offset_;
  }
 private:
  void onData(const char* data, int len) {
    //ssize_t pwrite(int fd, const void *buf, size_t count, off_t offset);
    int fd = fileno(get_pointer(out_));
    int num = ::pwrite(fd, data, len, offset_);
        assert(num > 0);
    offset_ += num;
  }
...
class Downloader : boost::noncopyable {
 public:
  Downloader(EventLoop* loop,
             const leaf::string& url)
      : loop_(loop),
        curl_(loop),
        url_(url),
 ...
 private:
  ...
  void onHeaderDone(curl::Request* c, int code) {
    LOG_DEBUG << code << " acceptRanges_: " << acceptRanges_ << " length: " << length_;
...
int main(int argc, char* argv[]) {
  ...
  leaf::string url = argc > 1 ? argv[1] : "http://chenshuo.com/pdf/MuduoManual.pdf";
